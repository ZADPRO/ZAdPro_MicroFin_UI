...
export default function Expense({}: Props) {
  ...
  const editButton = (rowData: any) => {
    return (
      <div className="flex w-full justify-center">
        <b
          onClick={() => {
            setUpdateExpense(true), setSelectedExpense(rowData)
          }}
        >
          <CiEdit
            size={'2rem'}
            className=" text-[green] hover:text-[white] hover:bg-[green] rounded-[15%]"
          />
        </b>
      </div>
    )
  ...
  return (
    <>
      <ToastContainer />
      <Header userName={username} pageName={'Expense'} reLoad={reLoadPage} />
      {loadingStatus ? (
        <div
          style={{
            display: 'flex',
        ...
        <div>
          ...
          <div className="flex justify-between m-3">
            <button
              className="bg-[green] hover:bg-[#008000da] text-white py-2 px-10 rounded-md"
              onClick={() => {
                setAddExpense(true)
              }}
            >
