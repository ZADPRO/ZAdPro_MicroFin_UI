            ...
            body={(pinCodeData) => {
              ...
              return (
                <div className="flex gap-2 items-center">
                  <Dropdown
                    value={null} // or a controlled state if needed
                    onChange={(e) => {
                      ...
                      if (action === 'create') {
                        ...
                        setMoveTargets((prev) => ({
                          ...prev,
                          [pinCodeData.refAreaPinCodeId]: null
                        }))
                      } else {
                        setMoveTargets((prev) => {
                          const updated = { ...prev }
                    ...
                    options={[
                      ...
                      { label: 'Move to New Area', value: 'create' },
                      { label: 'Move to Another Area', value: 'move' }
                    ]}
                    placeholder="Select Action"
                    className="w-full md:h-2rem text-sm align-items-center" // smaller text, padding
                    panelClassName="text-sm" // reduce dropdown menu text size
                  />

                  {isMoveSelected && (
                    <Dropdown
                      filter
                      value={moveTargets[pinCodeData.refAreaPinCodeId] ?? null}
                      onChange={(e) => {
                        const targetAreaId = e.value
                        setMoveTargets((prev) => ({
                          ...prev,
                      ...
                      options={groupedData
                        .filter((area) => area.refAreaId !== data.refAreaId)
                        .map((area) => ({
                          label: area.refAreaName,
                          value: area.refAreaId
                        }))}
