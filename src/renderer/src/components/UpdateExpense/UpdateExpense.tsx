...
export const UpdateExpense: React.FC<AddNewSupplierProps> = ({ closeSidebarNew, expenseData }) => {
    ...
    const handelSubmit = () => {
        ...
        if (status) {
            try {
                axios
                    .post(import.meta.env.VITE_API_URL + '/expense/expenseUpdate',
                        {
    ...
    return (
        <div>
            <ToastContainer />
            <div><p><b className='text-[1.2rem]'>Update Expense</b></p></div>
            <form onSubmit={(e) => { e.preventDefault(); handelSubmit() }}>
                <div className='m-5'>
                    <div className='flex justify-around py-2' >
                        <div className='flex flex-col w-[45%]'>
                            <label>Select Date</label>
                            <Calendar value={date} required placeholder='select Date' onChange={(e) => setDate(e.value)} dateFormat="dd / mm / yy" maxDate={new Date()} readOnlyInput />
                        </div>
                        <div className='flex flex-col w-[45%]'>
                            <label>Voucher Id</label>
                            <InputText placeholder='Enter Voucher ID' value={vocherId} required onChange={(e) => setVoucherId(e.target.value)} />
                        </div>
                    </div>
                    ...
                    <div className='flex justify-around py-2' >
                        ...
                        {
                            category === 0 && (<div className='flex flex-col w-[30%]'>
                                <label>Enter Category Name</label>
                                <InputText value={categoryName} required onChange={(e) => setCategoryName(e.target.value)} />
                        ...
                        <div className={`flex flex-col ${category === 0 ? 'w-[30%]' : 'w-[45%]'}`}>
                            ...
                            <InputText placeholder='Enter Sub-Category' value={subCategory} required onChange={(e) => setSubCategory(e.target.value)} />
                    ...
                    <div className='flex justify-around py-2' >
                        ...
                        <div className='flex flex-col w-[45%]'>
                            ...
                            <InputNumber inputId="currency-india" placeholder='Enter Amount' value={amount} required onValueChange={(e) => setAmount(e.value)} mode="currency" currency="INR" currencyDisplay="symbol" locale="en-IN" />
                        </div>
                        <div className='flex flex-col w-[45%]'>
                            <label>Select Amount Source</label>
                            <Dropdown filter value={bank} onChange={(e) => {
                                setBank(e.value)

                            }} options={bankOption}
                                required optionLabel="label"
                                placeholder="Select Amount Source" className="w-full" />
                        </div>
                    </div>
                    <div className='flex justify-around py-2' >

                        <div className='flex flex-col w-[40%]'>
                            <button className='bg-[green] py-2 text-[white] rounded-lg' type='submit'>Update Expense</button>
                        </div>
